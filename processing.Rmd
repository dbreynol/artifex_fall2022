...
Use the functions described above (<code>str_sub</code>, <code>str_locate</code>, and <code>str_split</code>) to obtain the following cleaned up data frame.
...
The output from assigment one gets us pretty close to our desired analytic dataset. However, it turns out that for our analysis we would like the data to be in *long* format. 
